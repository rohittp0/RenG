...
`https://github.com/rohittp0/RenG`. In-memory signing is enabled only when signing properties are
present. The `LocalTest` repository is rooted at the repository's `build/local-maven`, while the optional `R2` repository consumes the existing endpoint, bucket, and credential inputs. Configuration-cache exclusions remain limited to remote Maven publishing tasks.
...
- HTTP 404 means no complete public release exists, so the declaration governs.
...
The dependency-free static site follows Rentile's structure but documents only installation, the six targets, version and release behavior, and RenG's pre-runtime status. It uses `https://rohittp.com/reng/` as its canonical base. Maven versions are loaded dynamically from public metadata; the site and README contain no checked-in RenG version literal. API documentation waits for Cycle B.
